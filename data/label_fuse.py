...
def get_num_examples():
    return len(list(filter(lambda x: x.endswith('jpg'), os.listdir('fuse'))))

def read_pose(object_name, example_id):
    filename = os.path.join('fuse', '{}_info.pkl'.format(example_id))
    with open(filename, 'rb') as f:
        pkl = pickle.load(f)
        idx = linemod_cls_names.index(object_name)
        RT = pkl[1][idx]
        fuse_begin = pkl[0][idx]
        R = np.matrix(RT[:, :3])
        T = np.matrix(RT[:, -1]).transpose()
    return R, T, fuse_begin
...
def main(args):
    ...
    def project(P, R, T, fuse_begin):
        P_RT = R * P + T
        p = K * P_RT
        x = int(round(p[0, 0] / p[2, 0])) + fuse_begin[1]
        y = int(round(p[1, 0] / p[2, 0])) + fuse_begin[0]
        return (x, y, P_RT[2, 0])
    ...
    os.makedirs('{}_fuse_labels'.format(args.object_name), exist_ok=True)

    for example_id in range(num_examples):
        print('example {}/{}'.format(example_id, num_examples), end='\r')
        R, T, fuse_begin = read_pose(args.object_name, example_id)
        ...
        sample = example_id == 0
        if sample:
            img = cv2.imread('fuse/{}_rgb.jpg'.format(example_id))
        mask = np.zeros((args.img_h, args.img_w), dtype=np.uint8)
        for P_idx, P in enumerate(P_list):
            P_prime = P_prime_list[P_idx]
            (x, y, z) = project(P, R, T, fuse_begin)
            (x_prime, y_prime, _) = project(P_prime, R, T, fuse_begin)
            if y >= 0 and y < args.img_h and x >= 0 and x < args.img_w:
                if is_filled[y, x] == 0 or z_buffer[y, x] > z:
                    ...
                    mask[y, x] = 255
        mask = fill(mask)
        cv2.imwrite('{}_fuse_labels/mask{}.png'.format(args.object_name, example_id), mask)
        ...
        np.save('{}_fuse_labels/cor{}.npy'.format(args.object_name, example_id),
                correspondences)
        if sample:
        ...
        for keypt_idx in range(keypts_3d.shape[0]):
            (x, y, _) = project(keypts_3d[keypt_idx].reshape((3, 1)), R, T, fuse_begin)
            keypts_2d[example_id, keypt_idx, 0] = x
            keypts_2d[example_id, keypt_idx, 1] = y
    np.save(os.path.join('{}_fuse_labels'.format(args.object_name), 'keypoints_2d.npy'), keypts_2d)
