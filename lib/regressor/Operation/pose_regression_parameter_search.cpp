...
void PoseRegressionParameterSearch::Refinement(
  ...
  for (unsigned iter = 0; iter < para_config.max_num_iterations; ++iter) {
    ...
    for (unsigned i = 0; i < para_config.num_samples; ++i) {
      ...
      /*
      mat_x_dif[i][0] = pr_para.beta_edge - x_cur[0];
      mat_x_dif[i][1] = pr_para.beta_sym - x_cur[1];
      mat_x_dif[i][2] = pr_para.alpha_kpts - x_cur[2];
      mat_x_dif[i][3] = pr_para.alpha_edge - x_cur[3];
      mat_x_dif[i][4] = pr_para.alpha_sym - x_cur[4];
      */
    ...
    for (unsigned i = 0; i < para_config.num_samples; ++i) {
      ...
      for (int j = 0; j < 5; ++j) {
        for (int k = j; k < 5; ++k) {
          //matJ[i][off] = mat_x_dif[j] * mat_x_dif[k];
          matJ(off, i) = mat_x_dif.col(j).transpose() * mat_x_dif.col(k);
          off = off + 1;
        }
      }
    }
    ...
    for (unsigned search_id = 0; search_id < 20; ++search_id) {
      ...
      printf("search_iter = %d, loss_reduction = %f\n", iter, val_dif_next_median);
      updated = true;
      break;
    }
    if (updated == false) {
